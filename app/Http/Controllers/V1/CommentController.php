...
class CommentController extends BaseController
{
    ...
    /**
     * @api {get} /posts/{postId}/comments (post comment list)
     * @apiDescription (post comment list)
     * @apiGroup Post
     * @apiPermission none
    ...
    public function index($postId, Request $request)
    {
        ...
        if ($currentCursor !== null)
        {
            ...
            if ($comments->count())
            {
                ...
                $cursorPagination = new Cursor($currentCursor, $prevCursor, $nextCursor, $comments->count());
                return $this->response->collection($comments, new CommentTransformer(), [], function ($resource) use ($cursorPagination) {
                    $resource->setCursor($cursorPagination);
                });
            }
    ...
    /**
     * @api {post} /posts/{postId}/comments (create post comment)
     * @apiDescription (create post comment)
     * @apiGroup Post
     * @apiPermission jwt
    ...
    /**
     * @api {delete} /posts/{postId}/comments/{id} (delete post comment)
     * @apiDescription (delete post comment)
     * @apiGroup Post
     * @apiPermission jwt
     * @apiVersion 0.1.0
     * @apiSuccessExample {json} Success-Response:
